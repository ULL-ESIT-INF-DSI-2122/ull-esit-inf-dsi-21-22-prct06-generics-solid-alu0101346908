...
describe('Combat class tests', () => {
  ...
  const combat10: Combat = new Combat(artoria, artoria);
  const combat11: Combat = new Combat(lancelot, lancelot);
  const combat12: Combat = new Combat(jeanneDArc, jeanneDArc);
  const combat13: Combat = new Combat(percival, percival);
  const combat14: Combat = new Combat(aak, aak);
  const combat15: Combat = new Combat(ceobe, ceobe);
  ...
  const combat28: Combat = new Combat(pokemon2, pokemon2);
  const combat29: Combat = new Combat(pokemon1, pokemon1);
  const combat30: Combat = new Combat(pokemon4, pokemon4);
  const combat31: Combat = new Combat(pokemon, pokemon);
  const combat32: Combat = new Combat(pokemon2, pokemon1);
  const combat33: Combat = new Combat(exusiai, exusiai);
  const combat34: Combat = new Combat(hellagur, hellagur);
  ...
  it('Debe existir un metodo para iniciar una pelea ', () => {
    ...
    expect(combat11.start()).to.eql(2);
    expect(combat12.start()).to.eql(2);
    expect(combat13.start()).to.eql(2);
    expect(combat14.start()).to.eql(2);
    expect(combat15.start()).to.eql(2);
    expect(combat16.start()).to.eql(1);
    expect(combat17.start()).to.eql(1);
    expect(combat18.start()).to.eql(2);
    expect(combat19.start()).to.eql(1);
    expect(combat20.start()).to.eql(2);
    expect(combat21.start()).to.eql(2);
    expect(combat22.start()).to.eql(2);
    expect(combat23.start()).to.eql(2);
    expect(combat24.start()).to.eql(2);
    expect(combat25.start()).to.eql(1);
    expect(combat26.start()).to.eql(2);
    expect(combat27.start()).to.eql(2);
    expect(combat28.start()).to.eql(2);
    expect(combat29.start()).to.eql(2);
    expect(combat30.start()).to.eql(2);
    expect(combat31.start()).to.eql(2);
    expect(combat32.start()).to.eql(2);
    expect(combat33.start()).to.eql(2);
    expect(combat34.start()).to.eql(2);
  });
});
